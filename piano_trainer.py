...
def train_piano_melody(right, sequence_length=8, temperature=1.0, epochs=1):
	piano_melody = []
	...
	melody_generator.fit(train_melody_X, train_melody_Y, 
							validation_set=0.1, batch_size=64, n_epoch=epochs)

	return melody_generator.generate(300, temperature=temperature, seq_seed=melody_seed)

def train_piano_accompany(left, sequence_length=20, temperature=1.0, epochs=4):
	pinao_accompany = []
	...
	accompany_generator.fit(train_accompany_X, train_accompany_Y, 
							validation_set=0.1, batch_size=64, n_epoch=epochs)
	return accompany_generator.generate(300, temperature=temperature, seq_seed=accompany_seed)
